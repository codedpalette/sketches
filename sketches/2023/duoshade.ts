...
for (let i = 0; i < numLayers; i++) {
  ...
  container.addChild(drawLayer(layerRotation));
}

run({ container }, params);

function drawLayer(rotation: number) {
  const container = new Container();

  const noiseFactor = random.real(0.001, 0.01);
  const cutoff = random.real(0.25, 0.5);
  const mask = drawMask(noiseFactor, cutoff);
  container.mask = mask;
  ...
  const lines = drawLines(lineStep, sat, bri);
...
function drawMask(noiseFactor: number, cutoff: number) {
...
function drawLines(lineStep: number, sat: number, bri: number) {
